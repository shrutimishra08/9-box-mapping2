...
url = "https://raw.githubusercontent.com/shrutimishra08/9-box-mapping/refs/heads/main/employee_data_100.csv"
try:
    data = pd.read_csv(url)
    required_cols = ['EmployeeID', 'Department', 'Diplomatic', 'Balanced', 'Sociable', 'Innovative', 
                     'Performance_H1', 'Performance_H2', 'Tenure']
    missing_cols = [col for col in required_cols if col not in data.columns]
    if missing_cols:
        st.error(f"Missing columns in dataset: {missing_cols}. Please check employee_data_100.csv.")
        st.stop()
    # Validate numeric columns
    for col in ['Diplomatic', 'Balanced', 'Sociable', 'Innovative', 'Performance_H1', 'Performance_H2']:
        if not pd.to_numeric(data[col], errors='coerce').notna().all():
            st.error(f"Non-numeric data found in column: {col}. Please ensure all values are numbers.")
            st.stop()
...
def get_succession_pipeline(data, personas):
    try:
        data['Potential_Score'] = data.apply(lambda row: 100 - min([np.sqrt(sum((np.array([row['Diplomatic'], row['Balanced'], 
                                                                                       row['Sociable'], row['Innovative']]) - 
                                                                              np.array([personas[row['Department']][m] for m in 
                                                                                        ['Diplomatic', 'Balanced', 'Sociable', 'Innovative']])) ** 2)) 
                                                                  for dept in personas], default=0), axis=1)
...
try:
    employees_df = data[['EmployeeID', 'Department', 'Diplomatic', 'Balanced', 'Sociable', 'Innovative', performance_col]].copy()
    ...
    if employees_df['Performance'].isna().any():
        ...
        st.stop()
    # Adjust Potential to ensure spread
    employees_df['Potential'] = employees_df.apply(lambda row: min(100, max(0, 100 - np.sqrt(sum((np.array([row['Diplomatic'], row['Balanced'], 
                                                                                                     row['Sociable'], row['Innovative']]) - 
                                                                                            np.array([personas[row['Best_Match']][m] for m in 
                                                                                                      ['Diplomatic', 'Balanced', 'Sociable', 'Innovative']])) ** 2)) * 1.5)), axis=1)
...
try:
    ...
    if not succession_pipeline.empty:
        ...
        st.warning("No succession pipeline data available due to an error.")
except Exception as e:
